...
func waitn(t *testing.T, pxa []*Paxos, seq int, wanted int) {
	...
	for iters := 0; iters < 30; iters++ {
		...
		decided := ndecided(t, pxa, seq)
		//fmt.Printf("Waitn iteration %d: decided: %d, wanted %d\n", iters, decided, wanted)
		if decided >= wanted {
			break
		}
		time.Sleep(to)
...
func waitmajority(t *testing.T, pxa []*Paxos, seq int) {
	//fmt.Printf("Majority size: %d\n", len(pxa)/2+1)
	waitn(t, pxa, seq, (len(pxa)/2)+1)
	//fmt.Println("returning from waitmajority")
}
